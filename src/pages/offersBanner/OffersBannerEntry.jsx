...
const OffersBannerEntry = () => {
  ...
  const { loader, message, success, getOfferOneBannerData } = reducerResponse;
  ...
  useEffect(() => {
    if (mode !== "Add") {
      dispatch(fetchOneOfferBanner(offerId));
    }
  }, []);
  ...
  useEffect(() => {
    if (getOfferOneBannerData?.data && mode !== "Add") {
      const resolveImagePath = () => {
        if (!getOfferOneBannerData?.data?.bannerImage) return null;

        let imagePath = getOfferOneBannerData?.data?.bannerImage;
        ...
        return typeof imagePath === "string"
          ? imagePath.startsWith("http")
            ? imagePath
            : `http://luxcycs.com:5500/${imagePath}`
          : null;
      };
      setBannerImage(resolveImagePath);
      // setTitle(getOfferOneBannerData?.title)
      // settype(getOfferOneBannerData?.type)
      setCtaText(getOfferOneBannerData?.data?.ctaText);
      setCtaLink(getOfferOneBannerData?.data?.ctaLink);
      setStatus(getOfferOneBannerData?.data?.status);
    }
        ...
        <Grid2 container columnSpacing={4} rowSpacing={3}>
          <Grid2 size={6}>
            <Typography variant="p" sx={{ fontWeight: "bold" }}>
              CTA Text <span style={{ color: "red" }}>*</span>
            </Typography>
            <br />
            <TextField
              id="ctaText"
              ...
              onChange={(e) => {
                setCtaText(e.target.value),
                  setErrorMsg({ ...errorMsg, ctaTextError: "" });
              }}
              value={ctaText ?? ""}
            />
            {errorMsg?.ctaTextError && (
              <Typography
                variant="span"
          ...
          <Grid2 size={6}>
            <Typography variant="p" sx={{ fontWeight: "bold" }}>
              CTA Link <span style={{ color: "red" }}>*</span>
            </Typography>
            <br />
            <TextField
              id="ctaLink"
              ...
              onChange={(e) => {
                setCtaLink(e.target.value),
                  setErrorMsg({ ...errorMsg, ctaLinkError: "" });
              }}
              value={ctaLink ?? ""}
            />
            {errorMsg?.ctaLinkError && (
              <Typography
                variant="span"
                sx={{ fontSize: "14px", color: "red", fontWeight: "bold" }}
              >
              ...
              <Select
                ...
                onChange={(e) => {
                  setStatus(e.target.value),
                    setErrorMsg({ ...errorMsg, statusError: "" });
                }}
                fullWidth
                displayEmpty
              >
